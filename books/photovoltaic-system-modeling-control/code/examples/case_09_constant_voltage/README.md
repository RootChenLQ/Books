# 案例9: 恒电压法

## 系统示意图（AI自动生成）

<table>
<tr>
<td width="58%">
<img src="case_09_constant_voltage_ai_diagram.png" alt="案例9: 恒电压法系统示意图" width="100%"/>
</td>
<td width="42%">

**AI大模型总结要点**

- 📋 案例概述：工程背景**:
- 🔬 理论基础：经验法则
- 💻 代码示例：基本CV实现
- 📊 算法特性：优点 ✅

> 该图由AI图像生成引擎根据案例描述自动创建，呈现输入条件、物理模型、控制策略与关键指标之间的关系，可作为阅读正文前的快速导览。

</td>
</tr>
</table>



## 📋 案例概述

**工程背景**:
CV(Constant Voltage)是最简单的MPPT算法:
- **基于经验法则** - MPP通常在0.76-0.8×Voc
- **极简单** - 1行代码即可实现
- **极快速** - 瞬时响应无需迭代
- **极低成本** - 适合简单系统

**学习目标**:
1. 理解CV算法原理
2. 掌握电压比例选择
3. 对比CV与P&O/INC
4. 了解适用场景

---

## 🔬 理论基础

### 经验法则

**核心原理**: MPP电压与Voc成比例

```python
Vmpp ≈ k × Voc

典型k值:
- 晶硅组件: k = 0.76-0.78
- 薄膜组件: k = 0.70-0.75
- 多晶硅:   k = 0.76-0.80
```

### 物理解释

**为什么MPP在0.76×Voc附近?**

从I-V曲线特性:
```python
P = V × I
Pmax 在 dP/dV = 0

经验统计:
- 大量测试数据显示
- 不同类型组件
- Vmpp/Voc 比值稳定在0.76-0.8
```

### 算法流程

```python
步骤1: 测量或估算Voc
     → 开路测量
     → 从数据手册获取
     → 根据温度/辐照度估算

步骤2: 计算参考电压
     Vref = k × Voc

步骤3: 控制工作在Vref
     → 恒定电压控制
     → 无需反馈调整
```

---

## 💻 代码示例

### 1. 基本CV实现

```python
from code.models.mppt_algorithms import ConstantVoltage, MPPTController

# 创建CV算法
cv = ConstantVoltage(
    voltage_ratio=0.76,  # MPP电压比例
    voc=36.0             # 开路电压(V)
)

# 创建控制器
controller = MPPTController(cv, v_min=0, v_max=40.0)

# MPPT循环(极简单!)
v_pv = 20.0
for step in range(100):
    i_pv = module.calculate_current(v_pv)
    
    # CV更新
    v_ref = controller.step(v_pv, i_pv)
    
    # 电压跟踪
    v_pv = v_pv + 0.5 * (v_ref - v_pv)
```python

### 2. 改进型CV(带温度补偿)

```python
from code.models.mppt_algorithms import ImprovedCV

# 创建改进型CV
cv_improved = ImprovedCV(
    voltage_ratio=0.76,
    voc=36.0,
    temp_coef=-0.003  # 温度系数(/°C)
)

# 使用时传入温度
v_ref = cv_improved.update(
    voltage=v_pv,
    current=i_pv,
    temperature=temperature  # 实时温度
)
```python

### 3. 不同比例对比

```python
ratios = [0.70, 0.76, 0.80]

for ratio in ratios:
    cv = ConstantVoltage(voltage_ratio=ratio, voc=voc)
    controller = MPPTController(cv, v_min=0, v_max=voc)
    
    # 运行MPPT
    # ...
    
    perf = controller.evaluate_performance(pmpp)
    print(f"k={ratio}: 效率={perf['efficiency']:.1f}%")
```matlab

---

## 📊 算法特性

### 优点 ✅

1. **极简单**
   - 1行代码: `Vref = 0.76 * Voc`
   - 无需复杂计算
   - 易于理解实现

2. **极快速**
   - 瞬时响应
   - 无需迭代
   - 无建立时间

3. **成本极低**
   - 不需要高性能MCU
   - 最低成本方案
   - 8位MCU即可

4. **无振荡**
   - 恒定电压输出
   - 稳定运行
   - 无功率波动

5. **可靠性高**
   - 逻辑极简单
   - 不会误判
   - 故障率低

### 缺点 ❌

1. **精度低**
   - 典型效率: 90-95%
   - 损失5-10%功率
   - 不是真正MPP

2. **不能跟踪**
   - 固定电压比例
   - 环境变化不跟踪
   - 老化不适应

3. **依赖Voc**
   - 需要准确Voc
   - Voc测量有误差
   - 温度影响大

4. **适应性差**
   - 不同组件k不同
   - 老化后k变化
   - 遮挡情况失效

---

## 🎛️ 参数调优

### 电压比例k选择

**经验值**:
| 组件类型 | k值范围 | 推荐值 |
|---------|---------|--------|
| 单晶硅 | 0.76-0.78 | 0.76 |
| 多晶硅 | 0.76-0.80 | 0.78 |
| 薄膜 | 0.70-0.75 | 0.73 |
| HIT | 0.78-0.80 | 0.79 |

**精确确定k值**:
1. 测试实际Vmpp和Voc
2. 计算 k = Vmpp / Voc
3. 使用该k值

### Voc获取方法

**方法1: 开路测量**(最准确)
```python
# 短暂断开负载
disconnect_load()
wait(100ms)  # 等待稳定
voc_measured = measure_voltage()
reconnect_load()

cv.set_voc(voc_measured)
```python

**方法2: 数据手册**(简单)
```python
# 从组件数据手册
voc_stc = 36.0  # STC条件下
cv = ConstantVoltage(voltage_ratio=0.76, voc=voc_stc)
```python

**方法3: 温度补偿**(精确)
```python
voc_25 = 36.0  # 25°C时
temp_coef = -0.0035  # /°C
temperature = 40.0  # 当前温度

voc_actual = voc_25 * (1 + temp_coef * (temperature - 25))
```python

### 定期更新Voc

```python
cv = ConstantVoltage(
    voltage_ratio=0.76,
    voc=36.0,
    update_voc=True,           # 启用更新
    voc_update_interval=100    # 每100步更新一次
)
```python

---

## 🔧 工程实现要点

### 1. Voc测量

**快速测量法**:
```python
def measure_voc_fast():
    """快速Voc测量(1秒内)"""
    # 1. 记录当前工作点
    v_work, i_work = get_current_point()
    
    # 2. 快速开路
    disconnect_load()
    wait(100ms)
    
    # 3. 测量Voc
    voc = measure_voltage()
    
    # 4. 恢复工作
    reconnect_load()
    set_voltage(v_work)
    
    return voc
```python

### 2. 温度补偿

```python
class CVWithTempComp:
    def __init__(self, voc_25, temp_coef=-0.0035):
        self.voc_25 = voc_25
        self.temp_coef = temp_coef
    
    def get_voc(self, temperature):
        return self.voc_25 * (1 + self.temp_coef * (temperature - 25))
    
    def get_vref(self, temperature, k=0.76):
        voc = self.get_voc(temperature)
        return k * voc
```python

### 3. 与P&O混合

**策略**: 启动用CV,稳定后用P&O

```python
class HybridMPPT:
    def __init__(self):
        self.cv = ConstantVoltage(voltage_ratio=0.76, voc=36.0)
        self.po = PerturbAndObserve(step_size=1.0)
        self.mode = 'CV'  # 初始用CV
        self.switch_time = 100  # 100步后切换
        self.step_count = 0
    
    def update(self, v, i):
        self.step_count += 1
        
        if self.step_count < self.switch_time:
            # 初期用CV快速启动
            return self.cv.update(v, i)
        else:
            # 后期用P&O精确跟踪
            if self.mode == 'CV':
                self.mode = 'P&O'
                self.po.v_ref = self.cv.v_ref  # 继承CV的电压
            return self.po.update(v, i)
```matlab

---

## 📈 性能对比

### CV vs P&O vs INC

| 指标 | CV | P&O | INC |
|------|-----|-----|-----|
| 跟踪效率 | 90-95% | 97-98% | 98-99% |
| 响应速度 | 极快(0步) | 中(10-15步) | 快(8-12步) |
| 稳态振荡 | 无 | 1-3% | <1% |
| 计算复杂度 | 极低 | 低 | 中 |
| 代码量 | ~10行 | ~50行 | ~100行 |
| 成本 | 极低 | 低 | 中 |
| 可靠性 | 极高 | 高 | 中 |
| 适应性 | 差 | 好 | 好 |

### 典型性能数据

**CV (k=0.76)**:
```
跟踪效率:   92.5%
响应时间:   <0.1s (瞬时)
稳态振荡:   0 W
建立时间:   0 步
```python

**不同k值影响**:
```
k=0.70:  88.5%效率 (偏离较远)
k=0.76:  92.5%效率 (最优)
k=0.80:  90.8%效率 (偏离)
```python

---

## 🎯 应用场景

### 适用场景 ✅

1. **简单光伏系统**
   - 小型户用(1-3kW)
   - 路灯/监控
   - 离网系统

2. **成本敏感应用**
   - 低成本逆变器
   - 充电控制器
   - 简易MPPT

3. **快速启动需求**
   - 频繁启停
   - 瞬时响应
   - 应急电源

4. **稳定环境**
   - 辐照度变化小
   - 温度相对恒定
   - 无遮挡

### 不适用场景 ❌

1. **高精度要求**
   - 大型电站
   - 高效率要求
   - 损失不可接受

2. **变化环境**
   - 云层频繁
   - 温度剧变
   - 部分遮挡

3. **高端系统**
   - 并网逆变器
   - 商业屋顶
   - 科研测试

4. **复杂配置**
   - 多串并联
   - 不同组件
   - 多峰P-V曲线

---

## 💡 思考题

1. **为什么MPP通常在0.76×Voc附近?这个比例会变吗?**

2. **CV算法在什么情况下性能最差?为什么?**

3. **如何确定适合自己组件的最佳k值?**

4. **CV能否与其他算法结合?如何结合?**

5. **老化10年后,k值会如何变化?**

---

## 📚 作业

### 基础题

**作业1**: 实现基本CV算法,测试不同k值的效率。

**作业2**: 测量实际组件的Vmpp/Voc比值。

**作业3**: 对比CV、P&O、INC的建立时间。

### 进阶题

**作业4**: 实现带温度补偿的改进型CV。

**作业5**: 设计CV+P&O混合算法,兼顾速度和精度。

**作业6**: 分析不同类型组件(单晶/多晶/薄膜)的最佳k值。

### 挑战题

**作业7**: 实现自适应CV,根据历史数据调整k值。

**作业8**: 设计一个算法,自动选择CV/P&O/INC。

**作业9**: 研究CV在多峰P-V曲线下的表现。

---

## ✅ 检查清单

完成本案例后,你应该能够:

- [ ] 理解CV算法的经验法则原理
- [ ] 知道为什么k通常是0.76-0.8
- [ ] 实现基本CV算法
- [ ] 选择合适的电压比例k
- [ ] 测量和更新Voc
- [ ] 实现温度补偿
- [ ] 对比CV与P&O/INC的优劣
- [ ] 判断CV的适用场景
- [ ] 设计CV+P&O混合算法

---

## 📖 扩展阅读

### 经典论文

1. **"Comparative Study of MPPT Algorithms"**
   - Renewable Energy, 2012

2. **"Constant Voltage Method for MPPT"**
   - Solar Energy, 2010

### 改进方案

- **Adaptive CV**: 自适应k值调整
- **Hybrid CV-P&O**: 混合算法
- **CV with Voc Tracking**: 实时Voc跟踪
- **Fuzzy CV**: 模糊控制k值

### 工业应用

- 小型充电控制器
- 路灯/监控MPPT
- 离网系统控制器
- 低成本逆变器

---

**案例9完成!** 🎉

**三种算法对比总结**:
```
CV:   最简单,最快,精度低 (90-95%)
P&O:  简单,中速,精度中 (97-98%)
INC:  复杂,快速,精度高 (98-99%)

选择建议:
- 简单系统 → CV
- 通用场合 → P&O
- 高精度 → INC
```

下一步: [案例10 - 模糊逻辑MPPT](../case_10_fuzzy_logic/README.md)
