# 案例17: 功率因数控制

## 系统示意图（AI自动生成）

<table>
<tr>
<td width="58%">
<img src="case_17_power_factor_control_ai_diagram.png" alt="案例17: 功率因数控制系统示意图" width="100%"/>
</td>
<td width="42%">

**AI大模型总结要点**

- 📖 教学目标：掌握并网逆变器的功率控制技术，理解有功功率和无功功率的解耦控制，实现功率因数调节。
- 🎯 核心理论：三相功率理论
- 💻 代码实现：功率计算
- 🧪 实验内容：实验1: 功率计算与PQ解耦

> 该图由AI图像生成引擎根据案例描述自动创建，呈现输入条件、物理模型、控制策略与关键指标之间的关系，可作为阅读正文前的快速导览。

</td>
</tr>
</table>



## 📖 教学目标

掌握并网逆变器的功率控制技术，理解有功功率和无功功率的解耦控制，实现功率因数调节。

**核心内容**:
1. 功率计算与PQ解耦原理
2. 有功功率控制
3. 无功功率控制
4. 功率因数调节

---

## 🎯 核心理论

### 1. 三相功率理论

#### 瞬时功率 (dq坐标系)

在同步旋转dq坐标系下：
\[
\begin{cases}
P = \frac{3}{2} (v_d i_d + v_q i_q) \\
Q = \frac{3}{2} (v_q i_d - v_d i_q)
\end{cases}
\]

#### 电网电压定向

当电网电压定向于d轴时 (\( v_q \approx 0 \))：
\[
\begin{cases}
P \approx \frac{3}{2} v_d i_d \\
Q \approx -\frac{3}{2} v_d i_q
\end{cases}
\]

**关键结论**:
- \( i_d \) 控制有功功率 P
- \( i_q \) 控制无功功率 Q
- P和Q完全解耦，可独立控制

### 2. 功率因数

#### 定义
\[
PF = \frac{P}{S} = \frac{P}{\sqrt{P^2 + Q^2}} = \cos\varphi
\]

其中：
- P: 有功功率 (W)
- Q: 无功功率 (Var)
- S: 视在功率 (VA)
- φ: 功率因数角

#### 类型

| Q值 | 功率因数 | 负载类型 | 相位关系 |
|-----|----------|----------|----------|
| Q = 0 | PF = 1.0 | 纯阻性 | 电压电流同相 |
| Q > 0 | 0 < PF < 1 | 感性 | 电流滞后电压 |
| Q < 0 | 0 < PF < 1 | 容性 | 电流超前电压 |

### 3. PQ解耦控制

#### 控制结构
```python
P参考 ──→ [功率计算] ──→ i_d参考 ──┐
                                  ├──→ [dq电流控制] ──→ 三相电压输出
Q参考 ──→ [功率计算] ──→ i_q参考 ──┘
```

#### 功率到电流转换

当 \( v_q = 0 \) 时：
\[
\begin{cases}
i_d^{ref} = \frac{P^{ref}}{1.5 v_d} \\
i_q^{ref} = -\frac{Q^{ref}}{1.5 v_d}
\end{cases}
\]

---

## 💻 代码实现

### 1. 功率计算

```python
from models.inverter_control import PowerController

# 创建功率计算器
power_calc = PowerController(V_grid=311.0)

# 计算功率
v_d, v_q = 311.0, 0.0  # 电网电压定向
i_d, i_q = 10.0, 5.0   # dq电流

P, Q, S, PF = power_calc.calculate_power(v_d, v_q, i_d, i_q)

print(f"有功功率: {P:.0f} W")
print(f"无功功率: {Q:.0f} Var")
print(f"视在功率: {S:.0f} VA")
print(f"功率因数: {PF:.3f}")
```python

### 2. 功率到电流转换

```python
# 功率参考
P_ref = 2000.0  # 2kW
Q_ref = 1000.0  # 1kVar

# 转换为电流参考
i_d_ref, i_q_ref = power_calc.power_to_current(P_ref, Q_ref, v_d=311.0)

print(f"i_d参考: {i_d_ref:.2f} A")
print(f"i_q参考: {i_q_ref:.2f} A")
```python

### 3. 功率因数到无功功率

```python
# 根据功率因数计算无功功率
P_ref = 2000.0
PF_ref = 0.95  # 功率因数0.95

Q_ref = power_calc.pf_to_power(P_ref, PF_ref)
print(f"所需无功功率: {Q_ref:.0f} Var")
```python

### 4. PQ控制器

```python
from models.inverter_control import PQController, SRFPLL

# 创建PLL (用于电网同步)
pll = SRFPLL(Kp=50.0, Ki=1000.0)

# 创建PQ控制器
pq_ctrl = PQController(
    V_grid=311.0,      # 电网电压幅值
    Kp_i=0.5,          # 电流环Kp
    Ki_i=100.0,        # 电流环Ki
    L=5e-3,            # 滤波电感
    omega=2*np.pi*50,  # 电网角频率
    P_limit=5000.0,    # 有功功率限幅
    Q_limit=2000.0     # 无功功率限幅
)

# 控制循环
dt = 1e-4
for _ in range(N):
    # PLL同步
    theta, omega, freq = pll.update(va, vb, vc, dt)
    
    # PQ控制
    v_a_out, v_b_out, v_c_out = pq_ctrl.update(
        P_ref, Q_ref,           # 功率参考
        va, vb, vc,             # 电网电压
        i_a, i_b, i_c,          # 电流反馈
        theta, dt               # PLL角度和时间步长
    )
    
    # 获取功率状态
    status = pq_ctrl.get_status()
    P_actual = status['power']['P']
    Q_actual = status['power']['Q']
    PF_actual = status['power']['PF']
```matlab

---

## 🧪 实验内容

### 实验1: 功率计算与PQ解耦

**实验目的**: 验证dq坐标系下的PQ解耦特性

**实验步骤**:
1. 设置电网电压 v_d=311V, v_q=0V
2. 测试不同i_d和i_q组合
3. 计算P、Q、S和PF
4. 验证解耦关系

**测试用例**:
| 工况 | i_d(A) | i_q(A) | 预期P(W) | 预期Q(Var) | 预期PF |
|------|--------|--------|----------|------------|--------|
| 纯有功 | 10.0 | 0.0 | 4665 | 0 | 1.0 |
| 纯无功 | 0.0 | 10.0 | 0 | ±4665 | 0.0 |
| 混合 | 10.0 | 5.0 | 4665 | ±2333 | 0.894 |

### 实验2: 有功功率控制

**实验目的**: 测试有功功率阶跃响应

**实验步骤**:
1. 设置Q_ref=0 (功率因数=1)
2. 有功功率阶跃: 1kW → 3kW → 2kW
3. 观察功率跟踪过程
4. 验证Q保持为0

**预期结果**:
- P快速跟踪参考值
- Q保持接近0
- 功率因数保持在1.0附近

### 实验3: 无功功率与功率因数

**实验目的**: 分析功率因数与无功功率的关系

**实验内容**:
1. 固定P=2kW
2. 改变Q值: 0, ±500, ±1000 Var
3. 计算视在功率S和功率因数PF
4. 绘制PF随Q变化曲线

**观察要点**:
- Q=0时PF最大
- |Q|增大，PF降低
- 感性负载 Q>0
- 容性负载 Q<0

---

## 📊 性能指标

### 1. 控制精度

| 指标 | 目标值 | 说明 |
|------|--------|------|
| 有功功率误差 | < 2% | 稳态精度 |
| 无功功率误差 | < 5% | 稳态精度 |
| 功率因数 | > 0.95 | 并网要求 |

### 2. 动态性能

| 指标 | 目标值 | 说明 |
|------|--------|------|
| 功率阶跃响应 | < 100ms | 调节时间 |
| 超调量 | < 10% | 动态响应 |
| PQ耦合度 | < 5% | 解耦性能 |

### 3. 并网标准 (IEEE 1547)

| 项目 | 要求 |
|------|------|
| 功率因数范围 | 0.95超前 ~ 0.95滞后 |
| 有功功率变化率 | < 10%/分钟 |
| 无功功率调节能力 | ±0.44 × P_rated |

---

## ⚙️ 工程应用

### 1. 并网模式选择

**恒功率因数模式**:
```python
P_ref = P_mppt  # 从MPPT获取
Q_ref = 0       # 功率因数=1
```python

**恒无功功率模式**:
```python
P_ref = P_mppt
Q_ref = Q_grid  # 电网要求的无功功率
```python

**电压支撑模式**:
```python
# 根据电网电压调节Q
V_grid_measure = measure_voltage()
if V_grid_measure < V_nominal * 0.95:
    Q_ref = Q_max  # 发出无功，支撑电压
elif V_grid_measure > V_nominal * 1.05:
    Q_ref = -Q_max  # 吸收无功，降低电压
else:
    Q_ref = 0
```python

### 2. 功率限幅策略

**视在功率限制**:
```python
S_rated = 5000.0  # VA

# 限制P和Q使得 sqrt(P^2 + Q^2) <= S_rated
if np.sqrt(P_ref**2 + Q_ref**2) > S_rated:
    # 按比例缩减
    ratio = S_rated / np.sqrt(P_ref**2 + Q_ref**2)
    P_ref *= ratio
    Q_ref *= ratio
```python

**优先级策略**:
```python
# 有功优先
P_ref = min(P_ref, P_limit)
Q_max_available = np.sqrt(S_rated**2 - P_ref**2)
Q_ref = np.clip(Q_ref, -Q_max_available, Q_max_available)
```python

### 3. 低电压穿越(LVRT)

```python
if V_grid < V_nominal * 0.9:
    # 进入LVRT模式
    P_ref = P_ref * (V_grid / V_nominal)  # 降低有功
    Q_ref = Q_max  # 最大无功支撑
```

---

## 🎓 作业练习

### 练习1: 功率计算验证
已知三相系统：v_d=220V, v_q=0V, i_d=15A, i_q=-8A
1. 计算P、Q、S
2. 计算功率因数
3. 判断负载类型(感性/容性)

### 练习2: 功率因数设计
要求光伏系统输出P=3kW，功率因数PF=0.98(滞后)：
1. 计算所需无功功率Q
2. 计算i_d和i_q (v_d=311V)
3. 设计控制策略

### 练习3: 性能对比
对比三种工况的性能：
- 工况1: PF=1.0 (Q=0)
- 工况2: PF=0.95 (感性)
- 工况3: PF=0.95 (容性)

分析：
1. 视在功率S的差异
2. 电流幅值的差异
3. 逆变器损耗的影响

### 练习4: 优先级策略
逆变器额定容量S=5kVA，当前MPPT输出P=4.5kW：
1. 计算可用无功功率范围
2. 如果电网要求Q=2kVar，如何处理？
3. 设计功率分配策略

---

## 📚 扩展阅读

### 进阶控制
- 虚拟同步机(VSG)控制
- 下垂控制
- 自适应功率因数控制

### 并网标准
- IEEE 1547-2018
- IEC 61727
- GB/T 19964

### 工程实践
- 防孤岛保护与功率控制协调
- 多机并联功率均分
- 智能电网Q(V)特性

---

## ❓ 常见问题

**Q1: 为什么dq坐标系能实现PQ解耦？**
A: dq坐标系将AC量转为DC量，在电网电压定向下，i_d和i_q分别独立控制P和Q，没有交叉耦合项。

**Q2: 功率因数调节有什么意义？**
A: 
- 提高电网电压质量
- 减小线路损耗
- 满足并网标准
- 支撑电网稳定

**Q3: 感性和容性功率因数的区别？**
A:
- 感性(Q>0): 电流滞后电压，吸收无功
- 容性(Q<0): 电流超前电压，发出无功
- 对电网影响不同

**Q4: 如何选择功率因数？**
A:
- 白天负载重: PF=1.0最佳(减少损耗)
- 电网电压低: 发出无功(Q>0)支撑电压
- 电网电压高: 吸收无功(Q<0)降低电压

**Q5: 功率限幅如何处理？**
A: 视在功率 \( S = \sqrt{P^2 + Q^2} \) 不能超过逆变器额定容量，需按优先级分配：
1. 有功优先（保证发电）
2. 剩余容量分配给无功
3. 必要时降低有功输出

---

## 📖 参考文献

1. Teodorescu, R., et al. "Grid Converters for Photovoltaic and Wind Power Systems"
2. IEEE Std 1547-2018 "Standard for Interconnection"
3. Blaabjerg, F., et al. "Control of Power Converters in AC Microgrids"
4. Akagi, H., et al. "Instantaneous Power Theory and Applications"

---

**实验时间**: 1.5-2小时  
**难度等级**: ⭐⭐⭐⭐  
**前置知识**: 案例14(电流控制), 案例16(PLL同步)
