# 案例15: 电压控制

## 系统示意图（AI自动生成）

<table>
<tr>
<td width="58%">
<img src="case_15_voltage_control_ai_diagram.png" alt="案例15: 电压控制系统示意图" width="100%"/>
</td>
<td width="42%">

**AI大模型总结要点**

- 📖 教学目标：掌握光伏并网逆变器的电压控制技术，包括直流母线电压控制和交流输出电压控制。
- 🎯 核心理论：直流电压控制
- 💻 代码实现：DC电压控制器
- 🧪 实验内容：实验1: 直流母线电压控制

> 该图由AI图像生成引擎根据案例描述自动创建，呈现输入条件、物理模型、控制策略与关键指标之间的关系，可作为阅读正文前的快速导览。

</td>
</tr>
</table>



## 📖 教学目标

掌握光伏并网逆变器的电压控制技术，包括直流母线电压控制和交流输出电压控制。

**核心内容**:
1. 直流母线电压控制原理
2. 交流电压双环控制结构
3. 前馈补偿与解耦控制
4. 参数整定与性能优化

---

## 🎯 核心理论

### 1. 直流电压控制

#### 控制目标
稳定直流母线电压 \( V_{dc} \)，抑制负载功率波动的影响。

#### 数学模型
直流侧功率平衡方程：
\[
C_{dc} \frac{dV_{dc}}{dt} = i_{pv} - i_{inv}
\]

其中：
- \( C_{dc} \): 直流侧电容
- \( i_{pv} \): 光伏输入电流
- \( i_{inv} \): 逆变器输入电流

#### PI控制器设计
\[
i_{ref} = K_p (V_{ref} - V_{dc}) + K_i \int (V_{ref} - V_{dc}) dt
\]

参数整定（根据期望带宽 \( \omega_c \)）：
\[
\begin{cases}
K_p = 2 C_{dc} \omega_c \\
K_i = C_{dc} \omega_c^2
\end{cases}
\]

#### 前馈补偿
\[
i_{ref} = i_{ref,PI} + \frac{P_{load}}{V_{dc}}
\]

**优点**: 提高负载扰动抑制能力，减小电压跌落。

---

### 2. 交流电压控制（双环）

#### 控制结构
```python
        +------+       +------+       +------+
V_ref → | 电压环 | → | 电流环 | → | 逆变器 | → V_out
        | (PI) |     | (PI) |     | +PWM |
        +------+       +------+       +------+
            ↑              ↑              ↑
            └──── V_meas ──┴──── I_meas ──┘
```

#### 电压外环（慢）
输出电流参考：
\[
i_{ref} = K_{p,v}(V_{ref} - V_c) + K_{i,v}\int(V_{ref} - V_c)dt
\]

带宽设计：\( f_v \approx 200 \text{Hz} \)

#### 电流内环（快）
输出电压指令：
\[
v_{out} = K_{p,i}(i_{ref} - i_L) + K_{i,i}\int(i_{ref} - i_L)dt
\]

带宽设计：\( f_i \approx 2 \text{kHz} \)

#### 解耦控制
LC滤波器动态：
\[
\begin{cases}
L \frac{di_L}{dt} = v_{out} - v_C - Ri_L \\
C \frac{dv_C}{dt} = i_L - i_{load}
\end{cases}
\]

**解耦前馈**:
- 负载电流前馈: \( i_{ref} \leftarrow i_{ref} + i_{load} \)
- 电容电压前馈: \( v_{out} \leftarrow v_{out} + v_C \)

**效果**: 改善动态响应，减小调节时间。

---

### 3. 三相双环控制（dq坐标系）

#### 系统结构
```python
       DC电压环        dq电流环
V_dc → [  PI  ] → i_d → [ PI + 解耦 ] → v_abc
                  i_q=0
```

#### 优势
- DC电压控制稳定母线
- dq电流控制实现解耦
- 独立调节有功/无功功率

---

## 💻 代码实现

### 1. DC电压控制器

```python
from models.inverter_control import DCVoltageController

# 参数设计
C_dc = 2000e-6  # 2000μF
omega_c = 2 * np.pi * 100  # 100Hz带宽
Kp = 2 * C_dc * omega_c
Ki = C_dc * omega_c ** 2

# 创建控制器
dc_ctrl = DCVoltageController(Kp, Ki, C_dc, v_nominal=400.0, i_limit=50.0)

# 运行控制
i_ref = dc_ctrl.update(
    v_ref=400.0,           # 参考电压
    v_measured=390.0,      # 测量电压
    dt=1e-4,               # 时间步长
    p_load=2000.0,         # 负载功率（前馈）
    enable_feedforward=True
)
```python

### 2. AC电压控制器（双环）

```python
from models.inverter_control import ACVoltageController

# 电压环参数
omega_v = 2 * np.pi * 200  # 200Hz
Kp_v = 2 * C * omega_v
Ki_v = C * omega_v ** 2

# 电流环参数
omega_i = 2 * np.pi * 2000  # 2kHz
Kp_i = L * omega_i
Ki_i = R * omega_i

# 创建控制器
ac_ctrl = ACVoltageController(
    Kp_v, Ki_v, Kp_i, Ki_i, 
    L=5e-3, C=20e-6, omega=2*np.pi*50,
    v_limit=50.0, i_limit=400.0
)

# 运行控制
v_out = ac_ctrl.update(
    v_ref=311.0,           # 220Vrms * √2
    v_measured=300.0,      # 测量电容电压
    i_measured=10.0,       # 测量电感电流
    dt=1e-4,
    i_load=5.0,            # 负载电流（前馈）
    enable_decoupling=True
)
```python

### 3. 三相双环控制器

```python
from models.inverter_control import DualLoopVoltageController

ctrl = DualLoopVoltageController(
    Kp_v=0.5, Ki_v=100, C_dc=2000e-6,      # 电压环
    Kp_i=0.5, Ki_i=100, L=5e-3, omega=2*np.pi*50,  # 电流环
    v_dc_nominal=400.0, i_limit=50.0, v_limit=400.0
)

# 运行控制
v_a, v_b, v_c = ctrl.update(
    v_dc_ref=400.0,          # DC电压参考
    v_dc_measured=390.0,     # 测量DC电压
    i_a=10.0, i_b=-5.0, i_c=-5.0,  # 三相电流
    theta=0.0,               # Park变换角度
    dt=1e-4,
    p_load=2000.0,           # 负载功率（前馈）
    i_q_ref=0.0,             # 无功电流参考
    enable_feedforward=True,
    enable_decoupling=True
)
```matlab

---

## 🧪 实验内容

### 实验1: 直流母线电压控制

**实验目的**: 验证DC电压控制器的稳定性和负载扰动抑制能力

**实验步骤**:
1. 设置系统参数 (C=2000μF, V_nom=400V)
2. 设计PI参数 (带宽100Hz)
3. 施加阶跃负载变化 (1kW → 2kW → 3kW)
4. 观察电压响应和调节时间

**预期结果**:
- 电压调整率 < 5%
- 稳态误差 < 1V
- 前馈补偿有效抑制扰动

### 实验2: 交流电压双环控制

**实验目的**: 测试双环控制的跟踪性能和负载适应性

**实验步骤**:
1. 设置LC滤波器参数 (L=5mH, C=20μF)
2. 设计双环参数 (电压环200Hz, 电流环2kHz)
3. 跟踪50Hz正弦参考
4. 施加负载突变 (10Ω → 5Ω)

**预期结果**:
- 电压跟踪误差 < 2%
- 负载突变时电压跌落小
- 解耦控制改善动态

---

## 📊 性能指标

### 1. DC电压控制性能

| 指标 | 目标值 | 说明 |
|------|--------|------|
| 稳态电压偏差 | < 1% | \|V_dc - V_ref\| / V_ref |
| 电压调整率 | < 5% | 负载变化时电压波动 |
| 调节时间 | < 50ms | 达到稳态的时间 |
| 超调量 | < 10% | 阶跃响应超调 |

### 2. AC电压控制性能

| 指标 | 目标值 | 说明 |
|------|--------|------|
| 稳态跟踪误差 | < 2% | RMS误差 |
| THD | < 3% | 总谐波畸变率 |
| 负载调整率 | < 5% | 负载变化时电压变化 |
| 动态响应时间 | < 2周期 | 负载突变恢复时间 |

### 3. 双环控制优势

| 对比项 | 单环控制 | 双环控制 |
|--------|----------|----------|
| 稳态精度 | 一般 | 高 |
| 动态响应 | 慢 | 快 |
| 负载扰动抑制 | 差 | 好 |
| 参数整定 | 简单 | 复杂 |

---

## ⚙️ 工程应用

### 1. 参数整定流程

**步骤1: 确定控制带宽**
- DC电压环: 50-200Hz (光伏MPPT的1/10)
- AC电压环: 100-500Hz (电网频率的2-10倍)
- AC电流环: 1-5kHz (开关频率的1/5-1/10)

**步骤2: 计算控制器参数**
- DC电压: \( K_p = 2C\omega_c, K_i = C\omega_c^2 \)
- AC电压: \( K_p = 2C\omega_v, K_i = C\omega_v^2 \)
- AC电流: \( K_p = L\omega_i, K_i = R\omega_i \)

**步骤3: 仿真验证**
- 阶跃响应 (超调、调节时间)
- 频率响应 (相位裕度 > 45°)
- 负载扰动 (电压跌落)

**步骤4: 实验微调**
- 适当降低增益提高稳定裕度
- 增加限幅保护
- 启用前馈和解耦

### 2. 前馈补偿设计

**DC侧前馈**:
\[
i_{ff} = \frac{P_{load}}{V_{dc}}
\]

**作用**: 根据负载功率直接补偿所需电流，减小反馈控制压力。

**AC侧前馈**:
- 负载电流前馈: 补偿负载电流扰动
- 电容电压前馈: 改善内环输出

### 3. 限幅与保护

```python
# 电流限幅
i_ref = np.clip(i_ref, -i_max, i_max)

# 电压限幅
v_out = np.clip(v_out, -v_max, v_max)

# 防饱和积分
if abs(v_out) >= v_max:
    integral = 0  # 停止积分
```

---

## 🎓 作业练习

### 练习1: 参数整定
已知系统参数 C_dc=1000μF，要求电压环带宽150Hz。
1. 计算Kp和Ki
2. 分析相位裕度
3. 绘制波特图

### 练习2: 前馈设计
设计一个DC电压控制器，要求：
1. 基础PI控制稳态误差<2V
2. 添加功率前馈
3. 对比前馈前后的负载扰动响应

### 练习3: 双环整定
设计AC双环控制器，LC参数为L=3mH, C=30μF:
1. 确定电压环和电流环带宽比例
2. 计算PI参数
3. 仿真验证负载突变响应

### 练习4: 解耦控制
推导AC双环控制的解耦前馈项：
1. 写出LC滤波器状态方程
2. 分析耦合项
3. 设计解耦补偿
4. 对比解耦前后的性能

---

## 📚 扩展阅读

### 控制理论
- 级联控制系统设计
- 前馈-反馈复合控制
- 抗扰动控制

### 工程实践
- 数字实现（离散化、采样延迟）
- 传感器噪声滤波
- 死区补偿

### 进阶算法
- 自适应电压控制
- 预测电压控制
- 滑模电压控制

---

## ❓ 常见问题

**Q1: 为什么使用双环控制而不是单环？**
A: 双环控制将电压控制和电流控制分离，电压环调节稳态精度，电流环提升动态响应，同时便于限流保护。

**Q2: 电压环和电流环的带宽如何选择？**
A: 电流环带宽应远大于电压环（通常10倍以上），保证电流环快速跟踪电压环的指令，避免环路耦合。

**Q3: 前馈补偿什么时候需要？**
A: 当负载功率波动大或对动态性能要求高时，前馈能显著减小电压跌落，提升系统性能。

**Q4: dq坐标系与αβ坐标系哪个更好？**
A: dq坐标系将AC量转为DC量，PI控制器可实现零稳态误差；αβ坐标系需要PR控制器。通常三相系统优先选dq。

---

## 📖 参考文献

1. Mohan, N., et al. "Power Electronics: Converters, Applications, and Design"
2. Yazdani, A., et al. "Voltage-Sourced Converters in Power Systems"
3. Teodorescu, R., et al. "Grid Converters for Photovoltaic and Wind Power Systems"
4. IEEE Std 1547-2018 "Standard for Interconnection and Interoperability"

---

**实验时间**: 2-3小时  
**难度等级**: ⭐⭐⭐⭐  
**前置知识**: 案例13(PWM), 案例14(电流控制)
