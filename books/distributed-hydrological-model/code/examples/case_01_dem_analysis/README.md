# 案例1：DEM分析与河网提取

## 系统示意图（AI自动生成）

<table>
<tr>
<td width="58%">
<img src="case_01_dem_analysis_ai_diagram.png" alt="案例1：DEM分析与河网提取系统示意图" width="100%"/>
</td>
<td width="42%">

**AI大模型总结要点**

- 📋 案例概述：本案例演示如何进行**DEM（数字高程模型）分析**和**河网提取**，是分布式水文建模的基础。
- 🎯 核心内容：DEM基础
- 💻 代码实现：生成合成DEM
- 📊 实验结果：地形特征

> 该图由AI图像生成引擎根据案例描述自动创建，呈现输入条件、物理模型、控制策略与关键指标之间的关系，可作为阅读正文前的快速导览。

</td>
</tr>
</table>



**难度等级**: ⭐  
**预计学习时间**: 2小时  
**前置知识**: 基础Python和NumPy

---

## 📋 案例概述

本案例演示如何进行**DEM（数字高程模型）分析**和**河网提取**，是分布式水文建模的基础。

### 学习目标

- 理解DEM的概念和应用
- 掌握地形分析方法（坡度、坡向）
- 学会D8流向算法
- 理解汇流累积计算
- 掌握河网提取方法
- 识别流域出口

### 案例特色

✨ **纯NumPy实现**：不依赖GDAL等复杂GIS库  
✨ **完整流程**：从DEM到河网的全过程  
✨ **可视化丰富**：5种专业地形图表  
✨ **算法清晰**：D8流向和汇流累积详解

---

## 🎯 核心内容

### 1. DEM基础

**什么是DEM？**

数字高程模型（Digital Elevation Model）是地表高程的数字化表示，通常为规则网格格式。

**DEM数据结构**：
```python
高程矩阵(m):
  100  95  90  85  80
   98  93  88  83  78
   96  91  86  81  76
   94  89  84  79  74
   92  87  82  77  72
```

**DEM应用**：
- 地形分析（坡度、坡向、曲率）
- 流域划分
- 河网提取
- 汇流路径
- 地形指数计算

### 2. 地形分析

**坡度（Slope）**：
```python
坡度 = arctan(√(dz/dx)² + (dz/dy)²) × 180/π
```

**坡向（Aspect）**：
```matlab
坡向 = (90 - arctan2(-dy, dx) × 180/π) % 360
```
- 0° = 北
- 90° = 东
- 180° = 南
- 270° = 西

### 3. D8流向算法

**原理**：水流向8个邻近网格中坡降最大的方向。

**8个方向编码**：
```python
32  64  128
16  *   1
8   4   2
```

**算法步骤**：
1. 对每个网格
2. 计算到8个邻格的坡降
3. 选择坡降最大的方向
4. 赋予相应编码

### 4. 汇流累积

**定义**：流入某网格的上游网格总数。

**计算方法（递归）**：
```python
汇流累积[i,j] = 1 + Σ(所有流向[i,j]的上游网格的汇流累积)
```python

**物理意义**：
- 反映汇水面积
- 识别河道位置
- 估算流量大小

### 5. 河网提取

**方法**：设定汇流累积阈值
```python
河网 = 汇流累积 >= 阈值
```python

**阈值选择**：
- 太小：河网过密
- 太大：河网过疏
- 经验值：根据流域大小调整

---

## 💻 代码实现

### 1. 生成合成DEM

```python
def generate_synthetic_dem(nx=50, ny=50):
    """生成合成DEM"""
    x = np.linspace(0, 1, nx)
    y = np.linspace(0, 1, ny)
    X, Y = np.meshgrid(x, y)
    
    # 主趋势：从高到低
    trend = 100 * (1 - 0.7*X - 0.3*Y)
    
    # 添加山脊和山谷
    ridges = 20 * np.sin(3 * np.pi * X) * np.sin(2 * np.pi * Y)
    
    # 添加噪声
    noise = 5 * np.random.randn(ny, nx)
    
    dem = trend + ridges + noise
    return np.maximum(dem, 0)
```python

### 2. D8流向

```python
def d8_flow_direction(dem):
    """D8流向算法"""
    # 8个方向
    directions = [
        (0, 1, 1),    # 东
        (1, 1, 2),    # 东南
        (1, 0, 4),    # 南
        (1, -1, 8),   # 西南
        (0, -1, 16),  # 西
        (-1, -1, 32), # 西北
        (-1, 0, 64),  # 北
        (-1, 1, 128)  # 东北
    ]
    
    flow_dir = np.zeros_like(dem, dtype=int)
    
    for i in range(1, ny-1):
        for j in range(1, nx-1):
            max_slope = -np.inf
            max_dir = 0
            
            for di, dj, code in directions:
                ni, nj = i + di, j + dj
                distance = np.sqrt(di**2 + dj**2)
                slope = (dem[i,j] - dem[ni,nj]) / distance
                
                if slope > max_slope:
                    max_slope = slope
                    max_dir = code
            
            flow_dir[i,j] = max_dir if max_slope > 0 else 0
    
    return flow_dir
```python

### 3. 汇流累积（递归）

```python
def flow_accumulation(flow_dir):
    """汇流累积计算"""
    ny, nx = flow_dir.shape
    flow_acc = np.ones((ny, nx), dtype=int)
    visited = np.zeros((ny, nx), dtype=bool)
    
    def accumulate(i, j):
        if visited[i, j]:
            return flow_acc[i, j]
        
        visited[i, j] = True
        total = 1
        
        # 查找所有流向当前网格的上游
        for di, dj, code in directions:
            ni, nj = i - di, j - dj
            if 0 <= ni < ny and 0 <= nj < nx:
                if flow_dir[ni, nj] == code:
                    total += accumulate(ni, nj)
        
        flow_acc[i, j] = total
        return total
    
    for i in range(ny):
        for j in range(nx):
            accumulate(i, j)
    
    return flow_acc
```matlab

---

## 📊 实验结果

### 地形特征
- 流域面积：0.250 km²
- 高程范围：0 - 114 m
- 平均坡度：25°
- 最大坡度：63°

### 水系特征
- 河网密度：0.68%
- 河网长度：0.17 km
- 提取阈值：20个网格

---

## 🎨 可视化说明

### 1. DEM图
- 颜色：绿（低）→黄→橙→褐（高）
- 等高线：黑色细线
- 统计信息框

### 2. 坡度坡向图
- 左：坡度（黄→橙→红）
- 右：坡向（HSV色轮）

### 3. 流向图
- 背景：DEM
- 箭头：蓝色，指向下游

### 4. 汇流累积图
- 颜色：白→蓝（log尺度）
- 深色：汇流大

### 5. 河网图
- 背景：DEM
- 河网：蓝色
- 出口：红色星号

---

## 💡 工程应用

### 1. 流域划分

基于DEM和河网，可以：
- 划分子流域
- 计算流域面积
- 确定汇流路径

### 2. 水文模拟

DEM分析为水文模型提供：
- 空间离散化基础
- 地形参数（坡度、坡长）
- 河网结构

### 3. 洪水分析

- 淹没分析
- 洪水演进路径
- 风险区识别

---

## 📝 练习题

### 基础练习
1. 修改DEM大小（nx=100, ny=100）
2. 调整河网阈值（10, 30, 50）
3. 计算不同阈值下的河网密度

### 进阶练习
1. 实现Dinf流向算法（多流向）
2. 添加填洼算法
3. 计算地形湿度指数（TWI）

### 挑战练习
1. 读取真实DEM数据（GeoTIFF格式）
2. 实现流域自动划分
3. 计算河道纵剖面

---

## 🔍 常见问题

### Q1: 为什么不用GDAL？

**A**: 本案例用纯NumPy实现，便于：
- 理解算法原理
- 降低依赖复杂度
- 适合教学演示

实际工程可使用GDAL、WhiteboxTools等专业工具。

### Q2: D8流向有什么局限？

**A**: 局限性：
- 只有8个方向（45°间隔）
- 单流向假设
- 平坦区域处理困难

改进方法：
- Dinf（多流向）
- D-infinity
- MFD（多流向分配）

### Q3: 如何选择河网阈值？

**A**: 选择方法：
- 经验值：根据流域大小
- 实地验证：对比实际河网
- 多尺度分析：不同阈值对比
- 统计方法：坡度-面积分析

### Q4: 汇流累积单位是什么？

**A**: 网格数（个）

换算为面积：
```
汇水面积 = 汇流累积 × 网格面积
         = flow_acc × dx × dy
```

---

## 📚 参考资料

### 经典教材
1. **《地理信息系统导论》** - Kang-tsung Chang
2. **《水文学》** - 芮孝芳

### 论文
1. O'Callaghan, J. F., & Mark, D. M. (1984). "The extraction of drainage networks from digital elevation data." *Computer Vision, Graphics, and Image Processing*, 28(3), 323-344.

2. Tarboton, D. G. (1997). "A new method for the determination of flow directions and upslope areas in grid digital elevation models." *Water Resources Research*, 33(2), 309-319.

### 软件工具
- **WhiteboxTools** - 开源地形分析
- **GRASS GIS** - 强大的GIS平台
- **TauDEM** - 地形分析工具
- **RichDEM** - Python地形库

---

## ✅ 学习检查清单

- [ ] 理解DEM的概念和数据结构
- [ ] 掌握坡度和坡向计算
- [ ] 理解D8流向算法原理
- [ ] 掌握汇流累积计算方法
- [ ] 理解河网提取流程
- [ ] 能够运行并修改代码
- [ ] 能够解读地形图表
- [ ] 理解算法的局限性
- [ ] 了解实际应用场景
- [ ] 能够进行参数调整

---

**案例1完成！** 🎉

下一步：案例2 - Thiessen多边形

---

**作者**: CHS-Books项目组  
**日期**: 2025-11-02  
**版本**: v1.0
